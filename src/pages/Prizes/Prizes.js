...
import { Button, Card, CardBody, Col, Container, Nav, NavItem, NavLink, Pagination, PaginationItem, PaginationLink, Row, TabContent, TabPane } from "reactstrap"
import BreadCrumb from "../../Components/Common/BreadCrumb"
import { deletePrize, getPrizes, getRedemptionHistory } from "src/store/prizes/actions"
...
const Prizes = (props) => {
	...
	useEffect(() => {
		dispatch(getPrizes())
		dispatch(getRedemptionHistory())
	}, [])
	const { prizes, error, page, limit, total, redemptionHistory } = useSelector(state => ({
		prizes: state.Prizes.prizes,
		error: state.Prizes.error,
		page: state.Prizes.page,
		limit: state.Prizes.limit,
		total: state.Prizes.total,
		redemptionHistory: state.Prizes.redemptionHistory
	}));
	...
	useEffect(() => {
	}, [page, limit, total, prizes, redemptionHistory])
				...
				<TabContent activeTab={activeTab}>
					...
					<TabPane tabId="2" className="py-2 ps-2">
						<table className="table">
							...
							<tbody>
								{redemptionHistory && redemptionHistory.length > 0 && redemptionHistory.map((d, dIndex) => (
									<tr key={dIndex}>
										<th>{d.date}</th>
										<td>{d.amount}</td>
										<td>{d.remark}</td>
